/*
 * CDG-Bench v1.2 - Synthetic ARM64 Vulnerability Benchmark
 *
 * Changes from v1.1:
 *   - V2 PATCHED: msg_process_beta now has bounds check on index
 *   - V5 INTRODUCED: msg_cleanup now frees buffer then accesses it (UAF)
 *   - V1 patched (from v1.1), V3/V4 remain vulnerable, V6 still absent
 *
 * Compile for ARM64:
 *   aarch64-linux-gnu-gcc -O1 -g -fno-inline -o cdg_bench_v12 cdg_bench_v1.2.c
 *
 * Compile for x86_64 (testing):
 *   gcc -O1 -g -fno-inline -o cdg_bench_v12 cdg_bench_v1.2.c
 */
...
int msg_process_beta(const uint8_t *raw_msg, size_t msg_len) {
    ...
    /* v1.2 FIX: bounds check on index */
...
/* ============================================================
 * V3: CWE-125 — Still vulnerable in v1.2
 * ============================================================ */
...
/* ============================================================
 * V4: CWE-787 — Still vulnerable in v1.2
 * ============================================================ */
 ...
 * Bug: free(buffer) then access buffer[0] = use-after-free
 * ============================================================ */
void msg_cleanup(uint8_t *buffer, size_t len) {
    if (buffer != NULL) {
        memset(buffer, 0, len);
        free(buffer);       /* v1.2: free the buffer */
        buffer[0] = 0;     /* v1.2 BUG: use after free! <-- V5 HERE */
    }
}

/* ============================================================
 * V6: CWE-190 — Still safe in v1.2
 * ============================================================ */
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        printf("  IIII = config index (V1/V2: patched, V3: trigger if >= 0x0020)\n");
    ...
    /* Note: msg is freed inside msg_cleanup in v1.2, do NOT free again */
