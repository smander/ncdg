/*
 * CDG-Bench v1.3 - Synthetic ARM64 Vulnerability Benchmark
 *
 * Changes from v1.2:
 *   - V3 PATCHED: msg_process_gamma now has bounds check on hdr.index
 *   - V4 PATCHED: buffer_copy now checks length <= dest_size
 *   - V6 INTRODUCED: calc_offset uses uint16_t multiplication (overflow)
 *   - V1/V2 patched (from earlier), V5 (UAF) remains
 *
 * Compile for ARM64:
 *   aarch64-linux-gnu-gcc -O1 -g -fno-inline -o cdg_bench_v13 cdg_bench_v1.3.c
 *
 * Compile for x86_64 (testing):
 *   gcc -O1 -g -fno-inline -o cdg_bench_v13 cdg_bench_v1.3.c
 */
...
int msg_process_beta(const uint8_t *raw_msg, size_t msg_len) {
    ...
    if (index >= TABLE_SIZE_BETA) return -1;
...
/* ============================================================
 * V3: CWE-125 — PATCHED in v1.3
 * Bounds check added before pointer arithmetic access.
 * ============================================================ */
int msg_process_gamma(const uint8_t *raw_msg, size_t msg_len) {
    ...
    /* v1.3 FIX: bounds check on hdr.index */
    if (hdr.index >= TABLE_SIZE_GAMMA) return -1;
...
int buffer_copy(uint8_t *dest, size_t dest_size,
    ...
    /* v1.3 FIX: bounds check on length */
...
/* ============================================================
 * V5: CWE-416 (Use-After-Free) — Still vulnerable (from v1.2)
 * ============================================================ */
void msg_cleanup(uint8_t *buffer, size_t len) {
    if (buffer != NULL) {
        memset(buffer, 0, len);
        free(buffer);       /* v1.2: free the buffer */
        buffer[0] = 0;     /* BUG: use after free! <-- V5 HERE */
    }
}

/* ============================================================
 * V6: CWE-190 (Integer Overflow) — INTRODUCED in v1.3
 * Bug: uint16_t * uint16_t * uint16_t overflows intermediate
 * results before assignment to uint32_t return value.
 * ============================================================ */
uint32_t calc_offset(uint16_t base, uint16_t multiplier, uint16_t count) {
    /* v1.3 BUG: intermediate multiplication in uint16_t overflows */
    return base * multiplier * count;  /* <-- V6 HERE */
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        printf("  IIII = config index (V1/V2/V3: all patched)\n");
        printf("  LLLL = payload length (V4: patched)\n");
        printf("  CCCC = checksum\n");
        printf("  WARNING: msg_cleanup has use-after-free (V5)\n");
        printf("  WARNING: calc_offset has integer overflow (V6)\n");
        return 0;
    }
    ...
    /* Note: msg is freed inside msg_cleanup, do NOT free again */
