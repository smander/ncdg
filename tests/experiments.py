...
Usage:
  ...
  python3 experiments.py --exp 1   # Run specific experiment
"""
...
class Experiment2_PropagationAccuracy:
    ...
    def run(self):
        ...
        for level_name, constraint in sorted(variants.items()):
            ...
            propagated = g.propagate_detection(seed_id)
            found = cid in propagated
            
            sim = g.similarity(seed, constraint)
...
def run_all_experiments(binary_path: str = None):
    ...
    if binary_path is None:
        binary_path = os.path.join(os.path.dirname(__file__), "..", "cdg_bench_v10")
